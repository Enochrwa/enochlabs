...
| Hosting | Frontend: any static host (Vercel/Netlify/Cloudflare Pages). Backend: any container host (Render/Fly.io/Railway/a VPS via the provided `Dockerfile`). | Keeps both sides on generous free/cheap tiers appropriate for a bootstrapped business, while staying host-agnostic. |
